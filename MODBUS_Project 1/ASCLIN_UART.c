...
void send_buff(void)
{
    ...
    protocal_test[4] = 0x0f;         // data number
    //protocal_test[5] = pwm_data;            // data number

    crc_16_val.check_sum = CRC16_MODBUS(protocal_test, 5);


    protocal_test[5] = crc_16_val.b[0];
    protocal_test[6] = crc_16_val.b[1];
}

void rx_buff(void)
{
    if((uart_rx_buff[0] == 0xb8) && (uart_rx_buff[1] == 0xae))
    {
            if(uart_rx_buff[2] == 'O')
            {
                crc_16_val.check_sum = CRC16_MODBUS(uart_rx_buff, 5);
                if((crc_16_val.b[1] == uart_rx_buff[6]) && (crc_16_val.b[0] == uart_rx_buff[5]))
                        {
...
{
    /* Receive data via RX  */
    IfxAsclin_Asc_read(&g_ascHandle, uart_rx_buff, &rx_count, TIME_INFINITE);
    cnt_rcv++;
}
