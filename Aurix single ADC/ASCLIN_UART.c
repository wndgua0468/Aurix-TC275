...
void send_adc_data_TX(int int_data)
{
    Ifx_SizeT number_of_byte = 9;
    union
    {
            int data ;
            char bytedata[4];   // 여기서 int 는 4 byte 확인해야 함
    } m_adc_value;

    union
    {
            sint16 data ;
            char bytedata[2];    // 여기서 int 는 2 byte 확인해야 함
    }m_crc_value;

    m_adc_value.data = int_data;

    send_buff[0] = '#';

    send_buff[1] = 'I';
    send_buff[2] = m_adc_value.bytedata[0];
    send_buff[3] = m_adc_value.bytedata[1];
    send_buff[4] = m_adc_value.bytedata[2];
    send_buff[5] = m_adc_value.bytedata[3];

    send_buff[6] = m_crc_data.bytedata[0];
    send_buff[7] = m_crc_data.bytedata[1];
